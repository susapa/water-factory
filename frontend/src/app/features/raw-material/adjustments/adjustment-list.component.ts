...
import { Adjustment, StockLot } from '../../../core/models/raw-material-inventory.model';
...
export class AdjustmentListComponent implements OnInit {
  ...
  items = signal<Adjustment[]>([]);
  lots = signal<StockLot[]>([]);
  loading = signal(false);
  showDialog = false;
  viewOnly = false;
  editingAdj = signal<Adjustment | null>(null);
  searchText = signal('');
  ...
  typeOptions = [
    ...
    { label: 'เสียหาย (Damage)', value: 'damage' },
    { label: 'แก้ไข (Correction)', value: 'correction' },
  ];
  ...
  newLine() {
    return this.fb.group({
      rm_stock_lot_id: ['', Validators.required],
  ...
  openView(item: Adjustment) {
    ...
    this.svc.getAdjustment(item.id).subscribe(adj => {
      ...
      (adj.lines ?? []).forEach(l => {
        ...
        g.patchValue({ rm_stock_lot_id: l.rm_stock_lot_id, counted_qty: l.counted_qty, reason: l.reason });
        this.lines.push(g);
      });
      this.form.disable();
      this.showDialog = true;
  ...
  getLotLabel(lotId: string): string {
    ...
    return lot ? `${lot.lot_number} — ${lot.rm_code} (${lot.current_qty} ${lot.uom_code})` : lotId;
  }
